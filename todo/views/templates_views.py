...
class TodoListView(ListView):
    ...
    template_name = "todo/list.html"  # 데이터를 보여줄 HTML 템플릿 파일 지정
    context_object_name = (
        "todos"  # 템플릿에서 사용할 변수 이름 (기본값 object_list 대신 todos 사용)
    )
    ordering = [
        "-created_at"
...
class TodoCreateView(CreateView):
    model = Todo  # 이 뷰에서 사용할 모델 (Todo 테이블에 데이터 생성)
    fields = [
        "name",
        "description",
        "complete",
    ...
    template_name = "todo/create.html"  # Todo 생성 화면에 사용할 템플릿 파일
    success_url = reverse_lazy(
        "todo:list"
